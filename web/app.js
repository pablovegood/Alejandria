...
// ===========================
// 📌 Estado préstamos (para deshabilitar pedir en préstamo)
// ===========================
let activeLoans = 0;
let maxLoans = 4;
...
function updateBorrowButtonsState() {
  ...
  document.querySelectorAll(".loan-btn").forEach((btn) => {
    btn.disabled = isFull;
    btn.title = isFull
      ? `Has alcanzado el límite de préstamos (${maxLoans}). Devuelve uno para pedir otro.`
      : "📚 Tomar en préstamo";
  });
}
...
async function loadUserLoans() {
  ...
  try {
    ...
    // ✅ Si el backend devuelve meta (active/max), úsala. Si no, calcula.
    activeLoans = typeof data.active === "number" ? data.active : loans.length;
    maxLoans = typeof data.max === "number" ? data.max : 4;
    ...
    // ✅ Si ya estás a tope, deshabilita el botón de pedir en préstamo
    updateBorrowButtonsState();
  } catch (err) {
...
function renderResults(books, page = 1) {
  ...
  document.querySelectorAll(".loan-btn").forEach((btn) => {
    btn.addEventListener("click", async (e) => {
      ...
      // ✅ Si ha llegado al límite (403), avisar y bloquear UI
      if (res.status === 403) {
  ...
  // ✅ Aplica el estado final de los botones de préstamo en esta página
  updateBorrowButtonsState();
}
